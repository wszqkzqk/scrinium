...
class PipelineError(RuntimeError):
    ...
    The CLI boundary (``scrinium.cli.main``) converts this into a one-line
...
class InboxCtx:
    ...
    Attributes:
        ...
        meta: 提取后的 :class:`~scrinium.ingest.metadata.PaperMetadata`。
...
def step_office_convert(ctx: InboxCtx) -> StepResult:
    ...
    try:
        ...
        _log.error("MarkItDown 未安装，无法转换 Office 文件。请运行: pip install scrinium[office]")
        ctx.status = "failed"
...
def step_dedup(ctx: InboxCtx) -> StepResult:
    ...
    """
    from scrinium.ingest.metadata import enrich_metadata
...
def _process_inbox(
    ...
    *,
    ...
    if needs_mineru and not dry_run:
        from scrinium.ingest.mineru import check_server
        from scrinium.ingest.pdf_fallback import prefers_fallback_parser
    ...
    if use_cloud_batch and needs_mineru and not dry_run:
        from scrinium.ingest.mineru import _plan_cloud_chunking
        ...
        if pdfs_to_convert:
            from scrinium.ingest.mineru import ConvertOptions, convert_pdfs_cloud_batch
...
def batch_convert_pdfs(cfg: Config) -> dict[str, int]:
    ...
    from scrinium.ingest.mineru import ConvertOptions, check_server
    from scrinium.ingest.pdf_fallback import (
        convert_pdf_with_fallback,
        preferred_parser_order,
        prefers_fallback_parser,
    ...
    if prefer_fallback:
        ...
        from scrinium.ingest.mineru import convert_pdf
        ...
        if pdf_paths:
            with tempfile.TemporaryDirectory(prefix="scrinium_batch_") as tmp:
                tmp_dir = Path(tmp)
...
def _ingest_proceedings_ctx(ctx: InboxCtx, *, force: bool) -> bool:
    ...
    from scrinium.ingest.proceedings import ingest_proceedings_markdown
...
def _move_to_pending(
    ...
    *,
    ...
    """
    from scrinium.ingest.metadata import metadata_to_dict
    ...
    if not dir_name and ctx.meta and ctx.meta.title:
        from scrinium.ingest.metadata import generate_new_stem
...
def _ensure_registry_schema(conn, db_path: Path) -> None:
    ...
    try:
        ...
        _log.warning(
            "Duplicate publication_number values found; creating non-unique index. Run 'scrinium index' to rebuild."
        )
        conn.execute(
