...
用法：
    from scrinium.vectors import build_vectors, vsearch
...
from scrinium.config import _env_with_fallback
...
def _load_model(cfg: Config | None = None):
    ...
    if cfg is not None:
        ...
        device_cfg = "auto"
        source = "modelscope"
        hf_endpoint = _env_with_fallback("SCRINIUM_HF_ENDPOINT") or os.environ.get("HF_ENDPOINT") or ""
...
_GPU_PROFILE_FILE = Path("~/.cache/scrinium/gpu_profile.json").expanduser()
_LEGACY_GPU_PROFILE_FILE = Path("~/.cache/scholaraio/gpu_profile.json").expanduser()


def _gpu_profile_read_path() -> Path:
    """Return the GPU profile cache path to read, preferring the new location.

    Falls back to the legacy pre-fork path (``~/.cache/scholaraio/``) when only
    that file exists. Writes always go to the new location.
    """
    if _GPU_PROFILE_FILE.exists():
        return _GPU_PROFILE_FILE
    if _LEGACY_GPU_PROFILE_FILE.exists():
        _log.warning(
            "found legacy GPU profile cache %s; consider moving it to %s",
            _LEGACY_GPU_PROFILE_FILE,
            _GPU_PROFILE_FILE,
        )
        return _LEGACY_GPU_PROFILE_FILE
    return _GPU_PROFILE_FILE
...
def _load_or_create_profile(model, cfg: Config | None = None) -> dict:
    ...
    # Try loading from disk (new location, with legacy pre-fork fallback)
    profile_path = _gpu_profile_read_path()
    if profile_path.exists():
        try:
            all_profiles = json.loads(profile_path.read_text("utf-8"))
...
def _resolve_embed_api_base(cfg: Config | None = None) -> str:
    ...
    env_base = _env_with_fallback("SCRINIUM_EMBED_API_BASE").strip().rstrip("/")
    if env_base:
        return env_base
...
def _resolve_embed_api_key(cfg: Config | None = None) -> str:
    ...
    env_key = _env_with_fallback("SCRINIUM_EMBED_API_KEY")
    if env_key:
        return env_key
...
def _embed_batch_openai_compat(texts: list[str], cfg: Config | None = None) -> list[list[float]]:
    ...
    if not api_key:
        raise RuntimeError("未配置 embedding API key，请设置 embed.api_key 或 SCRINIUM_EMBED_API_KEY")
...
def build_vectors(papers_dir: Path, db_path: Path, rebuild: bool = False, cfg: Config | None = None) -> int:
    ...
    Args:
        ...
        cfg: 可选的 :class:`~scrinium.config.Config`，用于读取模型/设备配置。
...
def _build_faiss_from_db(
    ...
    ids_path: Path,
    *,
    empty_msg: str = "向量索引为空，请先运行 `scrinium embed`",
...
def vsearch(
    ...
    *,
    ...
    Args:
        ...
        cfg: 可选的 :class:`~scrinium.config.Config`，用于加载嵌入模型。
...
def _post_filter(
    ...
    *,
    ...
    """对向量检索结果做年份/期刊/类型过滤。"""
    from scrinium.papers import parse_year_range
