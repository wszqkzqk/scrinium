...
| 命令 | 用途 |
|------|------|
| `scholaraio tags [--json]` | 浏览词表：canonical 标签 + 别名 + 论文数 |
| `scholaraio tag "<paper-id>"` | 无标签参数：显示该论文已有标签 |
| `scholaraio tag "<paper-id>" <标签...>` | 加标签（别名自动归一为 canonical） |
| `scholaraio tag "<paper-id>" <标签...> --remove` | 移除标签 |
| `scholaraio show "<paper-id>" --layer 2` | 打标依据：L1 头部有"标签:"行，L2 有摘要 |
| `scholaraio search "<查询词>" --tag <标签>` | 按标签过滤（多个 --tag 为 AND）；`usearch` 同 |
| `scholaraio audit` | 报告 untagged（未打标）论文 |
...
- `scholaraio audit` 报告 untagged 论文，作为默认待打标清单
...
1. `scholaraio tag "<paper-id>"` 先看已有标签，不重复打
2. `scholaraio show "<paper-id>" --layer 2`（标题 + 摘要）；无摘要退回 `--layer 1`，仍不足以判断再看 L3 结论
3. 依据标题 + 摘要打 **2-5 个标签**，`scholaraio tag "<paper-id>" <标签...>` 写入
...
注意与 CLI 行为的关系：`scholaraio tag` 遇到新标签会**自动注册进词表并提示**——这是给临时单篇打标的便利；批量策展流程要求克制，新标签必须经用户审核后再用，防止词表在几百篇的批量操作中失控膨胀。

### 6. 收尾：词表健康度回顾

全部批次完成后：

1. 再跑 `scholaraio tags`，检查：
   - **过宽**：挂论文过多、失去区分度 → 建议拆分
...
```bash
scholaraio show "<paper-id>" --append-notes "## YYYY-MM-DD | curate | 策展深读
- 关键发现"
```
...
现有词表（必须优先复用，含别名）：
<scholaraio tags 输出的摘要>

工作流程：
1. 每篇先 `scholaraio tag "<paper-id>"` 看已有标签，再 `scholaraio show "<paper-id>" --layer 2`（无摘要则 --layer 1）
2. 依据标题+摘要打 2-5 个标签，组合方法/体系/任务/技术路线多个维度，优先复用词表已有标签：
   scholaraio tag "<paper-id>" <标签1> <标签2> ...
...
用户说："给库里没打标签的论文都打上标签"
→ 完整工作流：`scholaraio tags` 拉词表 → `scholaraio audit` 拿 untagged 清单 → 分批派 subagent → 收尾词表健康度回顾

用户说："把 rna-review 工作区的论文整理一下标签"
→ 范围限定到该 workspace，其余流程同上

用户说："给这篇打上 milestoning 和 enhanced-sampling"
→ 直接 `scholaraio tag "<paper-id>" milestoning enhanced-sampling`（临时单篇，可走 CLI 自动注册）

用户说："看看现在有哪些标签"
→ 执行 `scholaraio tags`

用户说："把这篇的 cryo-em 标签去掉"
→ 执行 `scholaraio tag "<paper-id>" cryo-em --remove`

用户说："找 free-energy 方向做了深度学习的论文"
→ 执行 `scholaraio search "深度学习" --tag free-energy --tag deep-learning`（AND 过滤）

## 纪律 checklist

- 打标前跑过 `scholaraio tags`，已有标签（含别名）被优先复用
- 每篇 2-5 个标签、多维度组合，没有过粗的单一标签
- subagent 未擅自注册词表外新标签；新标签提案已汇总交用户审核
- 深读超过 L2 的论文已按 T2 纪律写 notes
- 收尾已跑 `scholaraio tags` 回顾词表健康度，过宽/过窄/疑似重复标签已提合并建议
