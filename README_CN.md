...
```mermaid
flowchart TB
    U["用户 / AI agent"] --> S["技能 .agents/skills — 35 个工作流"]
    U --> C[scrinium CLI]
    S --> C
    C --> ING[摄入管线]
    C --> RET[检索]
...
Scrinium 可以先用最小配置跑起来，再按需要逐步补强。框架自身不调用任何 LLM 或嵌入模型——理解工作全部由你的 agent 完成。
...
.agents/skills/         # agent skills（AgentSkills.io 格式）
