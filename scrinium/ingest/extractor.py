...
if TYPE_CHECKING:
    from scrinium.config import Config, LLMConfig
    ...
    def extract(self, filepath: Path) -> PaperMetadata:
        ...
        if not self._api_key:
            ...
            raise RuntimeError(
                ...
                "或设置环境变量 SCRINIUM_LLM_API_KEY / DEEPSEEK_API_KEY。"
            )
...
def get_extractor(config: Config) -> MetadataExtractor:
    ...
    if mode == "llm":
        ...
        if not api_key:
            _log.warning("[LLM] no API key found; set SCRINIUM_LLM_API_KEY or llm.api_key in config.local.yaml")
